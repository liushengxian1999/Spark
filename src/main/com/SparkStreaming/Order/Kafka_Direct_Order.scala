...
object Kafka_Direct_Order {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val brokerList = "h1:9092,h2:9092,h3:9092"

    //指定zk的地址，后期更新消费的偏移量时使用(以后可以使用Redis、MySQL来记录偏移量)
    val zkQuorum = "h1:2181,h2:2181,h3:2181"
    ...
    val path = "D:\\Spark_Test\\IP\\data\\ip.txt"
    val ref = IPUtil.SetRules(ssc, path)
    ...
    kafkaStream.foreachRDD { kafkaRDD =>
      if(!kafkaRDD.isEmpty()){
        ...
        // 计算成交量总额
        OrderUtil.TotalPrice(fields)

        //计算每个省成交量
        OrderUtil.ProvinceAndPrice(ref,fields)

        //商品分类的成交总额
        OrderUtil.Product(fields)
